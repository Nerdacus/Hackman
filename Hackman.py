...
# Add boundary constraints for Mrs. Hackman
...
def move_mrs_hackman():
    ...
    if current_maze_layout == final_hacked_layout and mrs_hackman_actived:
        movement_speed = GRID_SIZE  # Set movement speed to GRID_SIZE or any other desired speed
        if mrs_hackman_direction == 'right':
            new_x = mrs_hackman_x + movement_speed
            if within_boundary(new_x, mrs_hackman_y) and new_x <= 740:  # Check boundary and right wall
                mrs_hackman_x = new_x
            else:
                mrs_hackman_direction = 'down'
                mrs_hackman_x -= 40  # Move 40 pixels inward from the right boundary

        elif mrs_hackman_direction == 'down':
            new_y = mrs_hackman_y + movement_speed
            if within_boundary(mrs_hackman_x, new_y) and new_y <= 460:  # Check boundary and bottom wall
                mrs_hackman_y = new_y
            else:
                mrs_hackman_direction = 'left'
                mrs_hackman_y -= 40  # Move 40 pixels inward from the bottom boundary

        elif mrs_hackman_direction == 'left':
            new_x = mrs_hackman_x - movement_speed
            if within_boundary(new_x, mrs_hackman_y) and new_x >= 60:  # Check boundary and left wall
                mrs_hackman_x = new_x
            else:
                mrs_hackman_direction = 'up'
                mrs_hackman_x += 40  # Move 40 pixels inward from the left boundary

        elif mrs_hackman_direction == 'up':
            new_y = mrs_hackman_y - movement_speed
            if within_boundary(mrs_hackman_x, new_y) and new_y >= 60:  # Check boundary and top wall
                mrs_hackman_y = new_y
            else:
                mrs_hackman_direction = 'right'
                mrs_hackman_y += 40  # Move 40 pixels inward from the top boundary
...
def main():
    global hackman_x, hackman_y, hackman_old_x, hackman_old_y, terminals, hacked_terminals, current_maze_layout, grid, score, ghost_x, ghost_y, ghost_speed, path_index, ghost_2_x, ghost_2_y, path_index_2, mrs_hackman_x, mrs_hackman_y
    # Main game loop
    ...
    while running:
        ...
        hackman_old_x = hackman_x
        hackman_old_y = hackman_y
        mrs_hackman_old_x = mrs_hackman_x
        mrs_hackman_old_y = mrs_hackman_y
